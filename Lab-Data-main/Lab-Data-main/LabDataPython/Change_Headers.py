...
GluUnacc = pd.read_csv("LabDataPython/RawData_Python_Glucose_Unacc.csv")
GluAcc = pd.read_csv("LabDataPython/RawData_Python_Glucose_Acc.csv")
HSMFW_mi_Acc = pd.read_csv("LabDataPython/RawData_Python_HSMFWmarine_Acc.csv")
HSMFW_mi_Unacc = pd.read_csv("LabDataPython/RawData_Python_HSMFWmarine_Unacc.csv")
HSMFW_Acc = pd.read_csv("LabDataPython/RawData_Python_HSMFW_Acc.csv")
HSMFW_Unacc = pd.read_csv("LabDataPython/RawData_Python_HSMFW_Unacc.csv")
...
HSMFW_Acc_Renamed.to_csv("LabDataPython/HSMFW_Acclimated_NewHeaders.csv")
df = HSMFW_Unacc
for v in VFA_HSMFW:
    for i in range(0,len(df.columns)-1):
        testname = df.columns[i]  
        ...
        if list_[0] == v:
            df = RenameCol_toPV(df,df.columns[i],"_",Power_per_Volume(float(list_[1]),2,3.9,.0397)+Power_per_Volume(float(list_[1]),2,0.4,.045))
HSMFW_Unacc_Renamed = df[df.columns.drop(list(df.filter(regex="_")))]
HSMFW_Unacc_Renamed.to_csv("LabDataPython/HSMFW_Unacclimated_NewHeaders.csv")




print(Power_per_Volume(200,2,3.9,.0397)+Power_per_Volume(200,2,0.4,.045))